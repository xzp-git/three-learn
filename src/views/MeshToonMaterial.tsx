...
const $: $Props = {
  ...
  datGui() {
    ...
    gui
      .add(this.mesh!.material! as THREE.MeshToonMaterial, "gradientMap", [
        "none",
        "three",
        "five",
      ])
      .onChange((val) => {
        if (val === "none") {
          ...
          (this.mesh!.material! as THREE.MeshToonMaterial).gradientMap = null;
        } else if (val === "three") {
          this.threeToOne?.dispose();
          const threeToOne = this.textureLoader!.setCrossOrigin(
            "anonymous"
          ).load(
            // 资源URL
          ...
          threeToOne!.magFilter = THREE.NearestFilter;
          (this.mesh!.material! as THREE.MeshToonMaterial).gradientMap =
            threeToOne!;
        } else {
          this.fiveToOne!.magFilter = THREE.NearestFilter;
          (this.mesh!.material! as THREE.MeshToonMaterial).gradientMap =
            this.fiveToOne!;
        }
  ...
  loadTextures() {
    ...
    this.texture = loader.setCrossOrigin("anonymous").load(
      // 资源URL
      "/src/assets/textures/textures/Glass_Vintage_001/Glass_Vintage_001_basecolor.jpg",
      // onLoad回调
      function (texture) {
